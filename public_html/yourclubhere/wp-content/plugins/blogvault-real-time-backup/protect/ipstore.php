...
class BVProtectIpstore_V636 {
	...
	function __construct($storage_type = BVProtectIpstore_V636::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == BVProtectIpstore_V636::STORAGE_TYPE_FS) {
			$this->storage = new BVProtectIpstoreFS_V636();
		} else {
			$this->storage = new BVProtectIpstoreDB_V636();
		}
	}

	public static function uninstall() {
		BVProtectIpstoreDB_V636::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == BVProtectIpstore_V636::STORAGE_TYPE_DB) {
