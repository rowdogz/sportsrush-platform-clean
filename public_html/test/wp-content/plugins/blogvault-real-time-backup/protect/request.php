...
if (!class_exists('BVProtectRequest_V636')) :
class BVProtectRequest_V636 {
	public $ip;
	public $host = '';
	public $uri;
	public $method = '';
	...
	public $files;
	public $respcode;
	public $status = BVProtectRequest_V636::STATUS_ALLOWED;
	public $category = BVProtectRequest_V636::CATEGORY_NORMAL;
	...
	public function __construct($ip_header, $config) {
		$this->ip = BVProtectUtils_V636::getIP($ip_header);
		$this->timestamp = time();
		...
		if ($this->can_decode_json) {
			if ($this->getContentType() === "application/json" && !empty($this->raw_body)) {
				$_json_params = BVProtectUtils_V636::safeDecodeJSON($this->raw_body,
	...
	public static function blacklistedCategories() {
		return array(
			BVProtectRequest_V636::CATEGORY_BOT_BLOCKED,
			BVProtectRequest_V636::CATEGORY_COUNTRY_BLOCKED,
			BVProtectRequest_V636::CATEGORY_USER_BLACKLISTED,
			BVProtectRequest_V636::CATEGORY_GLOBAL_BOT_BLOCKED
		);
	}

	public static function whitelistedCategories() {
		return array(BVProtectRequest_V636::CATEGORY_WHITELISTED);
	}
