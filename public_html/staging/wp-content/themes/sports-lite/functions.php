...
function get_results() {
    if ( ! class_exists( 'Football_Pool_Matches' ) ) {
        wp_send_json_error('Football Pool plugin is not active.');
    }

    $competition = isset($_POST['competition']) ? sanitize_text_field($_POST['competition']) : '';
    $round = isset($_POST['round']) ? sanitize_text_field($_POST['round']) : '';

    $matches_class = new Football_Pool_Matches();
    $results = $matches_class->get_matches($competition, $round);
    ...
    foreach ( $results as $result ) {
        ...
        echo '<h3>' . esc_html( $result['match'] ) . '</h3>';
        echo '<p>Score: ' . esc_html( $result['home_score'] ) . ' - ' . esc_html( $result['away_score'] ) . '</p>';
        echo '</div>';
    }
...
function enqueue_results_scripts() {
    wp_enqueue_script(
        'results-filter',
        get_template_directory_uri() . '/js/results-filter.js',
        array('jquery'),
        null,
        true
    );
    wp_localize_script('results-filter', 'resultsAjax', array('ajax_url' => admin_url('admin-ajax.php')));
}
add_action('wp_enqueue_scripts', 'enqueue_results_scripts');

// AJAX Handler for Filtering Results
function filter_results_callback() {
    global $wpdb;

    $competition = sanitize_text_field($_POST['competition']);
    $round = sanitize_text_field($_POST['round']);

    $query = "SELECT * FROM {$wpdb->prefix}matches WHERE 1=1";
    if ($competition) {
        $query .= $wpdb->prepare(" AND competition = %s", $competition);
    }
    if ($round) {
    ...
    if ($results) {
        foreach ($results as $result) {
            ...
            echo "<h3>{$result->home_team} vs {$result->away_team}</h3>";
            echo "<p>Score: {$result->home_score} - {$result->away_score}</p>";
            echo "</div>";
        }
    } else {
...
add_action('wp_ajax_filter_results', 'filter_results_callback');
add_action('wp_ajax_nopriv_filter_results', 'filter_results_callback');