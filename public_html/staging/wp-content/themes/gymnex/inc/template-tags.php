...
if ( ! function_exists( 'twenty_twenty_one_entry_meta_footer' ) ) {
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( is_sticky() ) {
				echo '<p>' . esc_html_x( 'Featured post', 'Label for sticky posts', 'Gymnex' ) . '</p>';
			}
			...
			if ( has_category() || has_tag() ) {
				...
				$categories_list = get_the_category_list( __( ', ', 'Gymnex' ) );
				if ( $categories_list ) {
					printf(
						/* translators: %s: list of categories. */
						'<div class="cat-links"><span class="fa_icon"><i class="fa fa-list-ul"></i></span>' . esc_html__( '%s', 'Gymnex' ) . ' </div>',
						$categories_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}

				/* translators: used between list items, there is a space after the comma. */
				$tags_list = get_the_tag_list( '', __( ', ', 'Gymnex' ) );
				if ( $tags_list ) {
					printf(
						/* translators: %s: list of tags. */
						'<div class="tags-links"><span class="fa_icon"><i class="fa fa-tag"></i></span>' . esc_html__( '%s', 'Gymnex' ) . '</div>',
	...
	function twenty_twenty_one_the_posts_navigation() {
		...
		the_posts_pagination(
			array(
				...
				'before_page_number' => esc_html__( 'Page ', 'Gymnex' ),
				'mid_size'           => 0,
				'prev_text'          => sprintf(
