	...
	class BVSecurityCallback extends BVCallbackBase {
		...
		public function setupWP2FA($secrets_by_uids, $to_encrypt, $cipher_algo, $enabled) {
			...
			foreach ($secrets_by_uids as $user_id => $secret) {
				...
				$result[$user_id][BVWP2FA::SECRET_META_KEY] = update_user_meta($user_id, BVWP2FA::SECRET_META_KEY, $secret_info);
				$result[$user_id][BVWP2FA::FLAG_META_KEY] = update_user_meta($user_id, BVWP2FA::FLAG_META_KEY, true);
			}

			if (is_bool($enabled)) {
				$config = array("enabled" => $enabled);
				$result[BVWP2FA::$wp_2fa_option] = $this->settings->updateOption(BVWP2FA::$wp_2fa_option, $config);
			}
		...
		public function deleteWP2FAKeys($user_ids, $is_disable = false) {
			...
			foreach ($user_ids as $user_id) {
				$secret_deleted = delete_user_meta($user_id, BVWP2FA::SECRET_META_KEY);
				$flag_deleted = delete_user_meta($user_id, BVWP2FA::FLAG_META_KEY);
				$result[$user_id] = array(
					BVWP2FA::SECRET_META_KEY => $secret_deleted,
					BVWP2FA::FLAG_META_KEY => $flag_deleted
				);
			}

			if ($is_disable === true) {
				$result[BVWP2FA::$wp_2fa_option] = $this->settings->deleteOption(BVWP2FA::$wp_2fa_option);
			}
