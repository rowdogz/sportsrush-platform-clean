...
if (!class_exists('BVProtectIpstoreFS_V581')) :
	class BVProtectIpstoreFS_V581 {
		...
		function __construct() {
			...
			$ips = BVProtectUtils_V581::parseFile($ip_store_file);
		...
		public function getTypeIfBlacklistedIP($ip) {
			return $this->getIPType($ip, BVProtectIpstoreFS_V581::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPBlacklisted($ip) {
			return $this->checkIPPresent($ip, BVProtectIpstoreFS_V581::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPWhitelisted($ip) {
			return $this->checkIPPresent($ip, BVProtectIpstoreFS_V581::IP_TYPE_WHITELISTED);
		}
		...
		private function getIPType($ip, $type) {
			switch ($type) {
			case BVProtectIpstoreFS_V581::IP_TYPE_BLACKLISTED:
				return isset($this->blacklisted_ips[$ip]) ? $this->blacklisted_ips[$ip] : null;
			case BVProtectIpstoreFS_V581::IP_TYPE_WHITELISTED:
