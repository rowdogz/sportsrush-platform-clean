...
if (!class_exists('BVProtectLP_V581')) :
class BVProtectLP_V581 {
	private $ip;
	private $time;
	private $ipstore;
	private $logger;
	...
	private $mode              = BVProtectLP_V581::MODE_DISABLED;
	...
	private $category          = BVProtectLP_V581::CATEGORY_ALLOWED;
	private $username          = '';
	private $message           = '';
	...
	private function __construct($request, $config, $brand_name) {
		...
		$this->ipstore = new BVProtectIpstore_V581();
		$this->logger = new BVProtectLogger_V581(BVProtectLP_V581::TABLE_NAME);
	...
	public static function uninstall() {
		BVProtect_V581::$db->dropBVTable(BVProtectLP_V581::TABLE_NAME);
	}
	...
	private function getCaptchaLink() {
		$account = BVAccount::apiPublicAccount(BVProtect_V581::$settings);
	...
	private function getAllowLoginsTransient() {
		return BVProtect_V581::$settings->getTransient('bvlp_allow_logins');
	}

	private function getBlockLoginsTransient() {
		return BVProtect_V581::$settings->getTransient('bvlp_block_logins');
	}
	...
	private function isProtecting() {
		return $this->mode === BVProtectLP_V581::MODE_PROTECT;
	}

	private function isActive() {
		return $this->mode !== BVProtectLP_V581::MODE_DISABLED;
	}
	...
	private function isUnBlockedIP() {
		$transient_name = BVProtectLP_V581::UNBLOCK_IP_TRANSIENT_PREFIX . $this->ip;
		$attempts = BVProtect_V581::$settings->getTransient($transient_name);

		if ($attempts && $attempts > 0) {
			BVProtect_V581::$settings->setTransient($transient_name, $attempts - 1, 600 * $attempts);
			return true;
		}
	...
	private function isLoginBlocked() {
		if ($this->getAllowLoginsTransient() ||
				($this->getLoginCount(BVProtectLP_V581::LOGIN_STATUS_FAILURE, null, $this->all_blocked_gap) < $this->block_all_limit)) {
			return false;
		}
	...
	private function terminateLogin() {
		...
		$this->log(BVProtectLP_V581::LOGIN_STATUS_BLOCKED);
	...
	public function loginInit($user, $username = '', $password = '') {
		if ($this->isUnBlockedIP()) {
			$this->category = BVProtectLP_V581::CATEGORY_UNBLOCKED;
		} else {
			$failed_attempts = $this->getLoginCount(BVProtectLP_V581::LOGIN_STATUS_FAILURE,
																							$this->ip, $this->failed_login_gap);

			if ($this->isWhitelistedIP()) {
				$this->category = BVProtectLP_V581::CATEGORY_BYPASSED;
			} elseif (BVProtectUtils_V581::isPrivateIP($this->ip)) {
				$this->category = BVProtectLP_V581::CATEGORY_PRIVATEIP;
			} elseif ($this->isBlacklistedIP()) {
				$this->category = BVProtectLP_V581::CATEGORY_BLACKLISTED;
				$this->terminateLogin();
			} elseif ($this->isKnownLogin()) {
				$this->category = BVProtectLP_V581::CATEGORY_BYPASSED;
			} elseif ($this->isLoginBlocked()) {
				$this->category = BVProtectLP_V581::CATEGORY_ALL_BLOCKED;
				$this->terminateLogin();
			} elseif ($failed_attempts >= $this->temp_block_limit) {
				$this->category = BVProtectLP_V581::CATEGORY_TEMP_BLOCK;
				$this->terminateLogin();
			} elseif ($failed_attempts >= $this->captcha_limit) {
				$this->category = BVProtectLP_V581::CATEGORY_CAPTCHA_BLOCK;
				$this->terminateLogin();
			}
		}
	...
	public function loginFailed($username) {
		...
		$this->log(BVProtectLP_V581::LOGIN_STATUS_FAILURE);
	}

	public function loginSuccess($username) {
		$this->username = $username;
		$this->message = 'Login Success';
		$this->log(BVProtectLP_V581::LOGIN_STATUS_SUCCESS);
	}

	private function isKnownLogin() {
		return $this->getLoginCount(BVProtectLP_V581::LOGIN_STATUS_SUCCESS,
																$this->ip, $this->success_login_gap) > 0;
	}

	private function getLoginCount($status, $ip, $gap) {
		$table = BVProtect_V581::$db->getBVTable(BVProtectLP_V581::TABLE_NAME);
		$query_str = "SELECT COUNT(*) as count from `$table` WHERE status=%d && time > %d";
		$query_args = array($status, ($this->time - $gap));

		$query = BVProtect_V581::$db->prepare($query_str, $query_args);
		if ($ip) {
			$query .= BVProtect_V581::$db->prepare(" && ip=%s", $ip);
		}

		$rows = BVProtect_V581::$db->getResult($query);
		if (!$rows) {
			return 0;
		}
