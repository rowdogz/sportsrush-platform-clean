...
if (!trait_exists('BVProtectFWRuleWPFunc_V581')) :
trait BVProtectFWRuleWPFunc_V581 {
	private function _rf_sanitizeUser() {
		...
		if (!function_exists('sanitize_user') || !BVProtectUtils_V581::haveMupluginsLoaded()) {
			throw new BVProtectRuleError_V581(
	...
	private function _rf_maybeSerialize() {
		...
		if (!function_exists('maybe_serialize') || !BVProtectUtils_V581::haveMupluginsLoaded()) {
			throw new BVProtectRuleError_V581(
	...
	private function _rf_isUserLoggedIn() {
		...
		if (!function_exists('is_user_logged_in') || !BVProtectUtils_V581::havePluginsLoaded()) {
			throw new BVProtectRuleError_V581(
	...
	private function _rf_getCurrentWPUser() {
		...
		if (!function_exists('wp_get_current_user') || !BVProtectUtils_V581::havePluginsLoaded()) {
			throw new BVProtectRuleError_V581(
				$this->addExState("getCurrentWPUser: Func wp_get_current_user doesn't exist.")
			);
		}

		return BVProtectFWRuleEngine_V581::toAllowedType(wp_get_current_user());
	}
	...
	private function _rf_currentUserCan() {
		...
		if (!function_exists('current_user_can') || !BVProtectUtils_V581::havePluginsLoaded()) {
			throw new BVProtectRuleError_V581(
	...
	private function _rf_getUserBy() {
		...
		if (!function_exists('get_user_by') || !BVProtectUtils_V581::havePluginsLoaded()) {
			throw new BVProtectRuleError_V581(
		...
		return BVProtectFWRuleEngine_V581::toAllowedType($user);
	}
	...
	private function _rf_getCurrentWPUserCapabilities() {
		...
		return BVProtectFWRuleEngine_V581::toAllowedType($user["allcaps"]);
	}
	...
	private function _rf_getOption() {
		...
		if (!function_exists('get_option') || !BVProtectUtils_V581::haveMupluginsLoaded()) {
			throw new BVProtectRuleError_V581(
				$this->addExState("getOption: Func get_option doesn't exist.")
			);
		}

		return BVProtectFWRuleEngine_V581::toAllowedType(get_option($option, $default_value));
	}
	...
	private function _rf_checkPasswordResetKey() {
		...
		if (!function_exists('check_password_reset_key') || !BVProtectUtils_V581::havePluginsLoaded()) {
			throw new BVProtectRuleError_V581(
	...
	private function _rf_wpUnslash() {
		...
		if (!function_exists('wp_unslash') || !BVProtectUtils_V581::haveMuPluginsLoaded()) {
			throw new BVProtectRuleError_V581(
