...
LOG_FILE = os.path.join(SCRIPT_DIR, "real-score-updater-rlcom-superleague.log")
...
_TIME_RE = re.compile(r"\b([01]?\d|2[0-3]):([0-5]\d)\b")  # 20:00 (24h)
_ORDINAL_RE = re.compile(r"(\d+)(st|nd|rd|th)", re.IGNORECASE)
_DATE_HEADING_RE = re.compile(
    r"\b(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+\d{1,2}(?:st|nd|rd|th)\s+[A-Za-z]+\s+\d{4}\b"
)
_RLCOM_ROUND_RE = re.compile(r"Round:\s*([0-9]+)\b", re.IGNORECASE)

# LeagueUnlimited <small> example:
# "Thu 12 Feb 8:00PM at LNER Community Stadium, York"
_LU_SMALL_RE = re.compile(
    r"^(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+(\d{1,2})\s+([A-Za-z]{3})\s+(\d{1,2}:\d{2})(AM|PM)\b",
    re.IGNORECASE
)
_LU_ROUND_HDR_RE = re.compile(r"\bRound\s+(\d+)\b", re.IGNORECASE)
...
def extract_rlcom_round_int(text: str):
    if not text:
        return None
    m = _RLCOM_ROUND_RE.search(text)
    if not m:
        return None
    try:
...
def load_valid_competitions_for_date(db, scrape_date: str):
    ...
    Only active, and within optional start/end date bounds.
    """
...
def detect_match_round_columns(cursor):
    """
    Detect whether pool_wpkl_matches has any of:
      - round (int)
      - round_name (varchar)
      - round_order (int)
    Returns dict flags.
    """
    cursor.execute("SHOW COLUMNS FROM pool_wpkl_matches")
    cols = {row[0] for row in cursor.fetchall()}
    return {
        "round": "round" in cols,
        "round_name": "round_name" in cols,
        "round_order": "round_order" in cols,
    }
...
def find_rlcom_fixture_blocks(soup: BeautifulSoup):
    blocks = []
    seen = set()
...
def scrape_rlcom_fixtures_for_date(scrape_date: str, comp_title: str, url: str):
    logging.info(f"🔍 Scraping RL.com: {comp_title} -> {url}")

    _, html = fetch_match_centre_html(url, timeout=30)
    if not html:
        logging.error(f"❌ Empty HTML retrieved for {comp_title} ({url})")
        return []

    soup = BeautifulSoup(html, "html.parser")
    blocks = find_rlcom_fixture_blocks(soup)
    ...
    for b in blocks:
        ...
        if not block_date or block_date != scrape_date:
            continue

        match_time = extract_time(block_text)
        round_int = extract_rlcom_round_int(block_text)
        ...
        fixtures.append(
            {
                "competition_bbc_title": comp_title,
                "home_team": home,
                "away_team": away,
                "match_time": match_time,
                "round": round_int,          # RL.com numeric round if present
                "round_name": None,
                "round_order": round_int,
                "venue": venue,
            }
        )

    logging.info(f"✅ {comp_title} fixtures for {scrape_date}: {len(fixtures)}")
    return fixtures


# ============================================================
# LEAGUEUNLIMITED SCRAPER (round-correct)
# ============================================================

def _lu_parse_small_datetime(s: str, year: int):
    s = clean_text(s)
    m = _LU_SMALL_RE.search(s)
    if not m:
        return None, None
    ...
    try:
        month = list(calendar.month_abbr).index(mon_abbr)
    except ValueError:
        return None, None
    if month == 0:
        return None, None

    dt = mydatetime.strptime(f"{year}-{month:02d}-{day:02d} {time_str}{ampm}", "%Y-%m-%d %I:%M%p")
    return dt.strftime("%Y-%m-%d"), dt.strftime("%H:%M")


def scrape_leagueunlimited_fixtures_for_date(scrape_date: str, comp_title: str, url: str):
    logging.info(f"🔍 Scraping LeagueUnlimited: {comp_title} -> {url}")

    _, html = fetch_match_centre_html(url, timeout=30)
    if not html:
        logging.error(f"❌ Empty HTML retrieved for LeagueUnlimited ({url})")
        return []

    soup = BeautifulSoup(html, "html.parser")
    target_year = int(scrape_date.split("-")[0])

    fixtures = []
    current_round_order = None
    current_round_name = None
    current_round_int = None

    # Walk DOM order:
    # - Whenever we hit a header containing "Round ...", set current_round_*
    # - Every <a.full-draw-game> until next round header belongs to that round
    for el in soup.find_all(True):
        # Round headers
        if el.name in ("h1", "h2", "h3", "h4", "h5", "h6"):
            hdr_text = clean_text(el.get_text(" ", strip=True))
            if "round" in hdr_text.lower():
                current_round_name = hdr_text
                m = _LU_ROUND_HDR_RE.search(hdr_text)
                if m:
                    try:
                        current_round_int = int(m.group(1))
                        current_round_order = current_round_int
                    except Exception:
                        current_round_int = None
                        current_round_order = None
                else:
                    # Cup style: "Quarter Final", etc.
                    current_round_int = None
                    # round_order will be set by "first seen" sequence if needed later
                    current_round_order = None

        # Games
        if el.name == "a" and "full-draw-game" in (el.get("class") or []):
            if not current_round_name:
                continue  # don't assign a nonsense round
            ...
            matchup = clean_text(strong.get_text(" ", strip=True))
            if " v " not in matchup:
                continue
            home_team, away_team = [clean_text(x) for x in matchup.split(" v ", 1)]

            game_date, game_time = _lu_parse_small_datetime(small.get_text(" ", strip=True), target_year)
            ...
            fixtures.append({
                ...
                "home_team": home_team,
                "away_team": away_team,
                "match_time": game_time,          # HH:MM local
                "round": current_round_int,       # numeric if present
                "round_name": current_round_name, # full header text
                "round_order": current_round_order,  # numeric if present (else None)
                "venue": None,                    # optional
            })
...
def upsert_fixtures_for_competition(db, fixtures, scrape_date: str, db_competition_name: str):
    cursor = db.cursor()
    col_flags = detect_match_round_columns(cursor)
    ...
    # If we have non-numeric rounds (cups), we still need a stable order.
    # We'll assign round_order in the order we *see* new round_name values.
    round_name_to_order = {}
    next_order = 1
    ...
    for fx in fixtures:
        try:
            ...
            # Find existing match near the same time by same teams + matchtype
            check_query = """
                SELECT id, play_date
            ...
            # Determine round fields
            round_int = fx.get("round")  # may be None
            round_name = fx.get("round_name")
            round_order = fx.get("round_order")

            # If round_order is missing but we have a round_name, assign it in first-seen sequence
            if round_order is None and round_name:
                if round_name not in round_name_to_order:
                    round_name_to_order[round_name] = next_order
                    next_order += 1
                round_order = round_name_to_order[round_name]

            updates = []
            params = []

            if existing:
                match_id, existing_play_date = existing

                if has_date_time_changed(existing_play_date, play_date_utc):
                    updates.append("play_date = %s")
                    params.append(play_date_utc)

                # Write round columns if present
                if col_flags["round"] and round_int is not None:
                    updates.append("round = %s")
                    params.append(int(round_int))

                if col_flags["round_name"] and round_name:
                    updates.append("round_name = %s")
                    params.append(round_name)

                if col_flags["round_order"] and round_order is not None:
                    updates.append("round_order = %s")
                    params.append(int(round_order))
                elif col_flags["round"] and round_int is None and round_order is not None:
                    # Fallback: if only 'round' exists, store order there
                    updates.append("round = %s")
                    params.append(int(round_order))
                ...
                cols = ["home_team_id", "away_team_id", "play_date", "matchtype_id", "stadium_id"]
                vals = [home_team_id, away_team_id, play_date_utc, matchtype_id, default_stadium_id]

                if col_flags["round"] and round_int is not None:
                    cols.append("round")
                    vals.append(int(round_int))
                elif col_flags["round"] and round_int is None and round_order is not None:
                    cols.append("round")
                    vals.append(int(round_order))

                if col_flags["round_name"] and round_name:
                    cols.append("round_name")
                    vals.append(round_name)

                if col_flags["round_order"] and round_order is not None:
                    cols.append("round_order")
                    vals.append(int(round_order))

                placeholders = ", ".join(["%s"] * len(cols))
                insert_query = f"INSERT INTO pool_wpkl_matches ({', '.join(cols)}) VALUES ({placeholders});"
...
def scrape_for_competition(scrape_date: str, comp_title: str, url: str):
    if not url:
        return []

    if "leagueunlimited.com" in url.lower():
        return scrape_leagueunlimited_fixtures_for_date(scrape_date, comp_title, url)

    # Default: RL.com scraper
    return scrape_rlcom_fixtures_for_date(scrape_date, comp_title, url)
...
def main(scrape_date: str):
    ...
    for bbc_title, cfg in comp_map.items():
        ...
        if not url:
            logging.info(f"⏭ No URL configured for '{bbc_title}' yet; skipping.")
            continue

        fixtures = scrape_for_competition(scrape_date, bbc_title, url)
