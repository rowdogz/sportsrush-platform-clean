...
class RumbleTalk_Public
{
    ...
    /**
     * creates a button that links to the chat when there is an admin available
    ...
    public function adminButton($attr = array())
    {
        ...
        if (!RumbleTalkSDK::validateHashStructure($attr['hash'])) {
            return "<div>Invalid room hash supplied</div>";
        }
        ...
        if (count($moderators) == 0) {
            $noAdminText = isset($attr['no-admin-text'])
                ? $attr['no-admin-text']
                : 'No admin available';
            $noAdminText = wp_kses($noAdminText, 'strip');
