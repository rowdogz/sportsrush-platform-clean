...
class RM_Options_Controller
{
    ...
    public function user($model, $service, $request, $params) {
        if (current_user_can('manage_options') || current_user_can('rm_options_managemanage_options')) {
        ...
        if (current_user_can('manage_options')) {
            if ($this->mv_handler->validateForm("options_admin_menu")) {
                if ($request->req['restore'] == 'false'){
                    ...
                    // Reload the page to reflect changes in menu
                    echo '<script type="text/javascript">window.location.href = "'.esc_url(admin_url( 'admin.php?page=rm_options_admin_menu' )).'";</script>';
                    /*
                    $data = $service->get_options();
