...
class Football_Pool_Pagination {
	...
	public function show_pages_line( $return = 'echo' ) {
		if ( $this->total_pages ) {
		...
		$output = sprintf(
			'<div class="tablenav"><div class="tablenav-pages%s"><span class="fp-pagination-line">',
			$page_class
		);
		$output .= sprintf( __( 'page %d of %d', 'football-pool' ), $this->current_page, $this->total_pages );
		$output .= '</span></div></div>';
