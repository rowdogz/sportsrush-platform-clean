...
function soccer_club_admin_notice() {
    ...
    $soccer_club_dismissed = get_user_meta(get_current_user_id(), 'soccer_club_dismissed_notice', true);
    $soccer_club_current_page = '';
    if(isset($_GET['page'])) {
    	$soccer_club_current_page = admin_url( "admin.php?page=".sanitize_text_field($_GET["page"]));
    }

    // Display the notice only if not dismissed
    if (!$soccer_club_dismissed && $soccer_club_current_page != admin_url( "admin.php?page=wordclever-templates")) {
        ?>
        		...
        		<div class="notice-content">
	        		<div class="notice-holder">
	                        ...
	                        <a href="javascript:void(0);" id="install-activate-button" class="button admin-button info-button">
							   <?php echo __('Getting started', 'soccer-club'); ?>
							</a>

							<script type="text/javascript">
							document.getElementById('install-activate-button').addEventListener('click', function () {
							    const soccer_club_button = this;
							    const soccer_club_redirectUrl = '<?php echo esc_url(admin_url("themes.php?page=soccer-club")); ?>';
	                        ...
	                        <a href="<?php echo esc_url( SOCCER_CLUB_DOC_URL ); ?>" target="_blank" id="doc-button" class="button admin-button bundle-button"><?php echo __('Free Documentation', 'soccer-club'); ?></a>
	            	</div>
	            </div>
